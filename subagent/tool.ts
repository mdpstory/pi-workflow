 ...
 *
 * Note: this module registers only the `subagent` tool. `wf_write_artifact`
 * is pi-workflow's own (role + CLR gated, routes shared artifacts to
 * .workflow/shared/) — registered in the top-level index.ts, strictly
 * better than the copy this tool used to ship, which is why that copy was
 * dropped during the merge into pi-workflow.
 */
...
export default function registerSubagentTool(pi: ExtensionAPI) {
	pi.registerTool({
		name: "subagent",
		label: "Subagent",
		get description() {
