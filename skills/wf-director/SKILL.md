...
2. **Resuming killed? Ask first.** `wf_status` read the **NEXT ACTION** line, **in-flight subagents** (avoid re-dispatch), **unread bus messages** (REJECTION BRIEF = read before retry), **last 3 discussion entries**, and **director memory** (`intent.md`) — your own durable first-person log. `wf_init` now blocks when the resolved workflow id already has real state (not all-todo). If you want to resume: tell the user, get their OK, then call `wf_init({ resume: true })`. To start fresh: `wf_new` then `wf_init`.
3. `wf_init({ resume: true })` to resume, or plain `wf_init()` for a fresh workflow. Creates `.workflow/` + stub artifacts.
